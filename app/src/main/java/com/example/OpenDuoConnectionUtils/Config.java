...
public class Config {
    ...
    private void registerPhoneAccount(Context context) {
        PhoneAccountHandle accountHandleIn = new PhoneAccountHandle(new ComponentName(context, OpenDuoConnectionService.class), Constants.PA_LABEL_CALL_IN);
        PhoneAccountHandle accountHandleOut = new PhoneAccountHandle(new ComponentName(context, OpenDuoConnectionService.class), Constants.PA_LABEL_CALL_OUT);
        PhoneAccount.Builder paBuilder;
        paBuilder = PhoneAccount.builder(accountHandleIn, Constants.PA_LABEL_CALL_IN).setCapabilities(PhoneAccount.CAPABILITY_CONNECTION_MANAGER);
        PhoneAccount phoneIn = paBuilder.build();
        paBuilder = PhoneAccount.builder(accountHandleOut, Constants.PA_LABEL_CALL_OUT).setCapabilities(PhoneAccount.CAPABILITY_CONNECTION_MANAGER);
